...
For most contributors, use the **[website paper-submission form](https://sait-crypto.github.io/Awesome-Social-Intelligence-Modeling-System/#contribute)**. It asks only for the paper basics, can import the full metadata record copied by the repository's Zotero plugin, and prepares a structured public GitHub issue without collecting an email address. A maintainer reviews the scope, duplicates, and taxonomy, then applies the `Action: Process` label to that issue. Missing DOI and category values receive safe intake placeholders so contributors are not required to understand the internal schema. The intake workflow creates a backend pull request and explicitly invokes the repository's existing paper-processing workflow, which validates the submission and updates the canonical databases. The accepted database commit then triggers one path-filtered website deployment.
