(() => {
  ...
  const elements = {
    ...
    uploadStatusTitle: document.querySelector("[data-upload-status-title]"),
    ...
    uploadRecoveryRetry: document.querySelector("[data-upload-recovery-retry]"),
    uploadRecoveryFallback: document.querySelector("[data-upload-recovery-fallback]"),
  };
      ...
      [...groupedTasks.entries()]
        ...
        .forEach(([subfieldId, groupTasks]) => {
          ...
          groupTasks.forEach((task) => {
            ...
            button.dataset.category = task.id;
            button.dataset.subfield = task.subfield;
  ...
  function updateTaskBarSelection(selectedCategory = "") {
    elements.taskBars.querySelectorAll(".task-bar[data-category]").forEach((button) => {
  ...
  function filterPapers({ resetLimit = true } = {}) {
    ...
    updateTaskBarSelection(values.category);
  ...
  function renderActiveFilters(values) {
    ...
    clearButton.textContent = t("filter.clear");
    clearButton.addEventListener("click", clearFilters);
    elements.activeFilters.append(clearButton);
  }
  ...
  function setupExplorerEvents() {
    ...
    elements.taskBars.addEventListener("click", (event) => {
      const button = event.target.closest(".task-bar[data-category]");
      if (!button || !elements.taskBars.contains(button)) return;
      const active = elements.category.value === button.dataset.category;
      setExplorerFilters(active
        ? { category: "", subfield: "", stage: "" }
        : {
            category: button.dataset.category,
            subfield: button.dataset.subfield,
  ...
  function markManualUploadRemainder(result, pipelineFiles, paperFile) {
    const uploadedItems = Array.isArray(result?.uploadedItems) ? result.uploadedItems : [];
    const uploadedPipelineIndexes = new Set(uploadedItems
      .filter((item) => item.kind === "pipeline")
      .map((item) => Number(item.index)));
    const paperUploaded = uploadedItems.some((item) => item.kind === "paper") || Boolean(result?.paperReference);
    return {
      ...(result || {}),
      manualUploadRequired: true,
      pendingPipelineIndexes: pipelineFiles
        .map((_file, index) => index)
        .filter((index) => !uploadedPipelineIndexes.has(index)),
      paperUploadRequired: paperFile instanceof File && paperFile.size > 0 && !paperUploaded,
    };
  }

  function pendingPipelineFilesFor(uploaded, pipelineFiles) {
    const indexes = Array.isArray(uploaded?.pendingPipelineIndexes)
      ? new Set(uploaded.pendingPipelineIndexes.map(Number))
      : new Set();
    return pipelineFiles.filter((_file, index) => indexes.has(index));
  }
  ...
  function buildIssueBody(formData, categories, uploaded = {}) {
    ...
    const manualUploadRequired = Boolean(uploaded.manualUploadRequired);
    const pendingPipelineCount = Array.isArray(uploaded.pendingPipelineIndexes)
      ? uploaded.pendingPipelineIndexes.length
      : 0;
    const paperUploadRequired = Boolean(uploaded.paperUploadRequired);
    const pipelineLines = [
      ...pipelineReferences,
      ...(pendingPipelineCount ? [
        `_Manual attachment required: paste or drag the ${pendingPipelineCount} remaining pipeline image${pendingPipelineCount === 1 ? "" : "s"} here before submitting this issue._`,
      ] : []),
    ];
    const manualParts = [
      pendingPipelineCount ? `${pendingPipelineCount} remaining Pipeline image${pendingPipelineCount === 1 ? "" : "s"}` : "",
      paperUploadRequired ? "the Paper file" : "",
    ].filter(Boolean);
    const fields = [
      ...
      [
        ...
        paperReference
          ? paperReference
          : paperUploadRequired
          ? "_Manual attachment required: drag the selected PDF here before submitting this issue._"
          : "_No response_",
      ],
    ...
    return [
      ...
      ...(manualUploadRequired ? [
        "> [!WARNING]",
        `> **Manual attachment required for ${manualParts.join(" and ")}.** Files already listed below were uploaded successfully and are kept. Add only the remaining files before selecting **Submit new issue**.`,
        "",
      ] : []),
  ...
  function chooseUploadRecovery({ completed, message, pendingFileName, total }) {
    ...
    if (!elements.uploadRecoveryDialog?.showModal) {
      return Promise.resolve(window.confirm(t("upload.recoveryConfirm", {
        message,
        progress,
        pending,
      })) ? "retry" : "fallback");
    }
    elements.uploadRecoveryMessage.textContent = message;
    elements.uploadRecoveryProgress.textContent = [progress, pending].filter(Boolean).join(" ");
    elements.uploadRecoveryDialog.showModal();
    return new Promise((resolve) => {
      let settled = false;
      const finish = (choice) => {
        if (settled) return;
        settled = true;
        elements.uploadRecoveryRetry.removeEventListener("click", retry);
        elements.uploadRecoveryFallback.removeEventListener("click", fallback);
        elements.uploadRecoveryDialog.removeEventListener("cancel", cancel);
        if (elements.uploadRecoveryDialog.open) elements.uploadRecoveryDialog.close();
        resolve(choice);
      };
      const retry = () => finish("retry");
      const fallback = () => finish("fallback");
      const cancel = (event) => event.preventDefault();
      elements.uploadRecoveryRetry.addEventListener("click", retry);
      elements.uploadRecoveryFallback.addEventListener("click", fallback);
      elements.uploadRecoveryDialog.addEventListener("cancel", cancel);
    });
  }

  function setUploadStatus(state = "", title = "", detail = "") {
    if (!elements.uploadStatus) return;
    const visible = Boolean(state && (title || detail));
    elements.uploadStatus.hidden = !visible;
    elements.uploadStatusTitle.textContent = title;
    elements.uploadStatusDetail.textContent = detail;
    if (visible) elements.uploadStatus.dataset.state = state;
    else delete elements.uploadStatus.dataset.state;
  }

  function setSubmissionBusy(busy) {
    elements.submitPaper.disabled = busy;
    elements.clearSubmission.disabled = busy;
    elements.submitPaper.setAttribute("aria-busy", String(busy));
    elements.submissionForm.setAttribute("aria-busy", String(busy));
    if (busy) elements.submissionForm.dataset.uploading = "true";
    else delete elements.submissionForm.dataset.uploading;
  }

  function updateUploadWaitingWindow(targetWindow, title, detail, state = "working") {
    if (!targetWindow || targetWindow.closed) return;
    try {
      const doc = targetWindow.document;
      doc.documentElement.lang = i18n.getLanguage();
      doc.title = title;
      let panel = doc.querySelector("[data-upload-wait-panel]");
      if (!panel) {
        ...
        style.textContent = `
          * { box-sizing: border-box; }
          body { margin: 0; min-height: 100vh; display: grid; place-items: center; padding: 24px; background: #f2f5f8; color: #1d2a3b; font-family: Inter, ui-sans-serif, system-ui, sans-serif; }
          .panel { width: min(560px, 100%); display: grid; grid-template-columns: 46px 1fr; gap: 18px; align-items: center; padding: 26px 28px; border: 1px solid #b8c9dc; border-radius: 12px; background: #fff; box-shadow: 0 18px 50px rgba(35, 55, 82, .14); }
          .mark { width: 38px; height: 38px; display: grid; place-items: center; border-radius: 50%; color: #315e93; font-size: 22px; font-weight: 750; }
          .panel[data-state="working"] .mark { border: 3px solid #d7e2ef; border-top-color: #315e93; animation: spin .85s linear infinite; }
          .panel[data-state="success"] .mark { background: #e7f2ed; color: #376c5b; }
          .panel[data-state="error"] .mark { background: #fff0ee; color: #a23b33; }
          h1 { margin: 0 0 7px; font-size: clamp(19px, 4.8vw, 24px); line-height: 1.2; }
          p { margin: 0; color: #536176; font-size: 15px; line-height: 1.55; }
          @keyframes spin { to { transform: rotate(360deg); } }
          @media (max-width: 480px) { .panel { grid-template-columns: 1fr; text-align: center; } .mark { margin: 0 auto; } }
        `;
        ...
        heading.dataset.uploadWaitTitle = "";
        const description = doc.createElement("p");
        description.dataset.uploadWaitDetail = "";
        copy.append(heading, description);
        panel = doc.createElement("main");
        panel.className = "panel";
        panel.dataset.uploadWaitPanel = "";
        panel.append(mark, copy);
        doc.body.replaceChildren(panel);
      }
      panel.dataset.state = state;
      panel.querySelector(".mark").textContent = state === "success" ? "✓" : state === "error" ? "!" : "";
      panel.querySelector("[data-upload-wait-title]").textContent = title;
      panel.querySelector("[data-upload-wait-detail]").textContent = detail;
    } catch (_error) {
      // The main page retains the same prominent progress state if the waiting tab cannot be styled.
    }
  }
  ...
  function setupSubmissionForm() {
    elements.submissionForm.addEventListener("submit", async (event) => {
      ...
      let manualUploadRequired = Boolean(hasFiles && !directUpload);
      let issueWindow = null;
      if (directUpload) {
        const preparingDetail = t("upload.preparingDetail", {
          total: pipelineFiles.length + (paperFile instanceof File && paperFile.size > 0 ? 1 : 0),
        });
        setUploadStatus("working", t("upload.preparingTitle"), preparingDetail);
        issueWindow = window.open("about:blank", "_blank");
        if (!issueWindow) {
          setUploadStatus("error", t("upload.popupBlockedTitle"), t("upload.popupBlocked"));
          releaseButton(0);
          return;
        }
        issueWindow.opener = null;
        updateUploadWaitingWindow(issueWindow, t("upload.waitingTitle"), t("upload.waitingPage"));
          ...
          try {
            uploaded = await window.SIMUpload.uploadSubmissionFiles({
              ...
              onProgress: ({ completed, total, file }) => {
                const state = file ? "working" : "success";
                const statusTitle = file ? t("upload.progressTitle") : t("upload.completeTitle");
                const statusDetail = file
                  ? t("upload.progress", { completed: completed + 1, total, name: file.name })
                  : t("upload.complete", { total });
                setUploadStatus(state, statusTitle, statusDetail);
                updateUploadWaitingWindow(issueWindow, statusTitle, statusDetail, state);
              },
            });
            break;
          } catch (error) {
            const timeoutSeconds = window.SIMUpload?.fileTimeoutSeconds || 45;
            const message = error?.name === "TimeoutError"
              ? t("upload.timeout", { seconds: timeoutSeconds })
              : error instanceof Error ? error.message : String(error);
            uploadState = error?.uploadState || uploadState;
            const partialResult = error?.partialResult || uploaded;
            const completed = Array.isArray(partialResult?.uploadedItems)
              ? partialResult.uploadedItems.length
              : 0;
            const total = Number(uploadState?.total) || pipelineFiles.length + (paperFile instanceof File && paperFile.size > 0 ? 1 : 0);
            const pendingFileName = String(uploadState?.pendingFileName || "");
            const pausedDetail = t("upload.recoveryStatus", { completed, message, total });
            setUploadStatus("error", t("upload.recoveryTitle"), pausedDetail);
            updateUploadWaitingWindow(issueWindow, t("upload.recoveryWaitingTitle"), t("upload.recoveryWaiting"), "error");
            const choice = await chooseUploadRecovery({ completed, message, pendingFileName, total });
            if (choice === "retry") {
              const retryDetail = t("upload.retrying", { completed, remaining: total - completed, total });
              setUploadStatus("working", t("upload.retryingTitle"), retryDetail);
              updateUploadWaitingWindow(issueWindow, t("upload.retryingTitle"), retryDetail, "working");
              continue;
            }
            manualUploadRequired = true;
            uploaded = markManualUploadRemainder(partialResult, pipelineFiles, paperFile);
            const fallbackDetail = t("upload.failedFallback", { message });
            setUploadStatus("error", t("upload.failedFallbackTitle"), fallbackDetail);
            updateUploadWaitingWindow(issueWindow, t("upload.failedFallbackTitle"), fallbackDetail, "error");
            break;
          }
        }
      } else if (hasFiles) {
        uploaded = markManualUploadRemainder({}, pipelineFiles, paperFile);
        setUploadStatus("notice", t("upload.manualFallbackTitle"), t("upload.manualFallback"));
      }

      const pendingPipelineFiles = manualUploadRequired
        ? pendingPipelineFilesFor(uploaded, pipelineFiles)
        : [];
      const paperUploadRequired = manualUploadRequired && Boolean(uploaded.paperUploadRequired);
      ...
      if (issueUrl.length > 7900 && navigator.clipboard?.writeText) {
        try {
          ...
          window.alert(manualUploadRequired
            ? `${t("submission.long")}\n\n${t("upload.manualIssueReminder")}`
            : t("submission.long"));
          const shortUrl = `${REPOSITORY_URL}/issues/new?${new URLSearchParams({ title }).toString()}`;
          if (issueWindow && !issueWindow.closed) issueWindow.location.replace(shortUrl);
      ...
      const imageCopy = pendingPipelineFiles.length === 1
        ? copyPipelineImage(pendingPipelineFiles[0])
        : null;
      rememberSubmissionDraft(fingerprint);
      if (issueWindow && !issueWindow.closed) issueWindow.location.replace(issueUrl);
      ...
      } else if (pendingPipelineFiles.length > 1) {
        attachmentInstructions.push(t("pipeline.attachMultiple", { count: pendingPipelineFiles.length }));
      }
      if (paperUploadRequired) {
  ...
  async function initializeData() {
    try {
      ...
      renderTaskBars(state.data.papers, controls().category);
      setupExplorerEvents();
      setupSubmissionForm();
