...
class ReadmeGenerator:
    ...
    def __init__(self):
        ...
        self.complete_list_database_path = self.settings['paths'].get(
            ...
            os.path.join(self.project_root, 'engineering', 'paper_database_complete_list.csv'),
        )
        ...
        self.paper_metadata_path = self.settings['paths'].get(
            'paper_metadata',
            os.path.join(self.project_root, 'engineering', 'config', 'paper_metadata.json'),
        )
    ...
    def _write_text_atomic(path: str, content: str) -> None:
        ...
        try:
            with tempfile.NamedTemporaryFile(
                ...
                dir=output_dir, prefix='.generate-', suffix='.tmp'
            ) as f:
                temp_path = f.name
                f.write(content)
    ...
    def generate_complete_list_content(self) -> str:
        ...
        return (
            '# Complete Paper List\n\n'
            '<!-- This file is generated by engineering/src/convert.py. Do not edit it manually. -->\n\n'
    ...
    def update_complete_list_file(self) -> bool:
        ...
        try:
            self._write_text_atomic(self.complete_list_output_path, content)
    ...
    def _generate_analogy_cell(self, paper: Paper) -> str:
        ...
        content = self._sanitize_field(value)
        return (
            '<details><summary>**[analogy]**</summary>'
            f'<div style="margin-top:6px">{content}</div></details>'
        )
    ...
    def _generate_pipeline_cell(self, paper: Paper) -> str:
        ...
        for raw_path in parts[:3]:
            ...
            if not os.path.exists(full_path) and normalized.startswith('assets/'):
                suffix = normalized[len('assets/'):]
                configured = os.path.join(self.assets_dir, suffix)
                full_path = configured if os.path.isabs(configured) else os.path.join(self.project_root, configured)
    ...
    def _generate_category_architecture(self) -> str:
        """Render the category architecture image, or a durable source placeholder."""
        configured = str(
            self.settings.get('readme', {}).get(
                'category_architecture_image',
                'engineering/assets/category_architecture.png',
            )
            or ''
        ).strip().replace('\\', '/')

        if not configured:
            return '<!-- CATEGORY_ARCHITECTURE_IMAGE: set readme.category_architecture_image -->'

        full_path = configured if os.path.isabs(configured) else os.path.join(self.project_root, configured)
        if os.path.isfile(full_path):
            try:
                image_path = os.path.relpath(full_path, self.project_root).replace('\\', '/')
            except Exception:
                image_path = configured
            return (
                '<!-- CATEGORY_ARCHITECTURE_START -->\n'
                '<p align="center">\n'
                f'<img src="{image_path}" width="95%" alt="Category architecture" align="center" />\n'
                '</p>\n'
        ...
        return (
            '<!-- CATEGORY_ARCHITECTURE_START\n'
            f'Add the category architecture image at: {configured}\n'
            'It will be displayed here automatically the next time README is generated.\n'
            'CATEGORY_ARCHITECTURE_END -->'
        )
    ...
    def update_readme_file(self, include_complete_list: bool = True) -> bool:
        ...
        category_architecture = self._generate_category_architecture()
        ...
        intro_parts = [part for part in (category_architecture, tables_intro) if part]
        if intro_parts:
