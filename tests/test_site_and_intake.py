...
class SurveyWebsiteBuildTests(unittest.TestCase):
    def test_build_produces_self_contained_site_from_selected_database(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertEqual(data["stats"]["total_paper_count"], len(complete["papers"]))
            self.assertEqual(stored["stats"]["total_paper_count"], len(complete["papers"]))
    ...
    def test_language_switch_is_shared_across_both_site_pages(self):
        ...
        for html in (homepage, complete):
            ...
            self.assertGreater(html.index('data-language-toggle'), html.index('</nav>'))
    ...
    def test_submission_form_exposes_provenance_image_and_duplicate_guard(self):
        ...
        self.assertIn('data-clear-pipeline', html)
        self.assertIn('data-clear-pipeline disabled', html)
        self.assertIn('novalidate', html)
        self.assertIn('data-i18n="form.doiHint"', html)
        self.assertIn("Prefer GitHub's native issue form?", html)
        self.assertIn("issues/new?template=paper_submission.yml", html)
...
class AutomationWiringTests(unittest.TestCase):
    def test_action_mode_silently_preserves_missing_paper_file_metadata(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            utils = UpdateFileUtils.__new__(UpdateFileUtils)
            utils.project_root = temp_dir
            utils.assets_dir = "engineering/assets"
            utils.legacy_figure_dir = "engineering/figures"
            utils.legacy_paper_dir = "engineering/papers"
            paper = Paper(
                uid="metadata-only",
            ...
            with (
                patch.dict(validate_submission.os.environ, {"IGNORE_PAPER_FILE_ASSETS": "true"}),
                patch("builtins.print") as output,
            ):
                normalized = utils.normalize_asset_fields(paper, ["paper_file"], strict=True)

        self.assertEqual(normalized.paper_file, "engineering/assets/metadata-only/local-paper.pdf")
    ...
    def test_accepted_update_triggers_pages_and_failure_mail_is_visible(self):
        ...
        self.assertIn("actions/upload-artifact@v7", processing)
        self.assertIn("actions/download-artifact@v8", processing)
        self.assertIn("enable-cache: false", processing)
        self.assertIn("Close source pull request without merging", processing)
        self.assertIn("state: 'closed'", processing)
        ...
        self.assertIn("notify-deployment:", deployment)
